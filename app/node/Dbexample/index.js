...
app.get("/api/sprayer",(req,res)=>{
    db.query("SELECT *FROM shops WHERE worktype='car spraying'",(err,result)=>{
        if (err){
            console.log(err)
        }
...
app.get("/api/car/hundaicars",(req,res)=>{
    db.query("SELECT *FROM shops WHERE expert='Hundai'",(err,result)=>{
        if (err){
            console.log(err)
        }
...
app.get("/api/car/toyotaCars",(req,res)=>{
    db.query("SELECT *FROM shops WHERE expert='Toyota'",(err,result)=>{
        if (err){
            console.log(err)
        }
...
app.post('/api/post',(req,res)=>{
    ...
    db.query('SELECT *FROM customers WHERE username =?',[name],
        (err,results)=>{
            if (err){
...
app.get('/people/:name',(req,res)=>{
    const name=req.params.name;
    db.query('SELECT *FROM Mechanics m JOIN Shops s ON m.mechanic_id = s.mechanic_id WHERE s.shop_name =?',[name],(error,result)=>{
        if(error){
...
app.post('/api/mechanics',(req,res)=>{
    ...
    const query=`INSERT INTO mechanics (name,email,password) VALUES ('${name}','${email}','${password}')`;
...
app.post('/api/mechanic_id',(req,res)=>{
    ...
    db.query('SELECT mechanic_id FROM mechanics WHERE name=? AND password=?',[name,password],(err,result)=>{
        if (err){
...
app.post('/api/addshop',(req,res)=>{
    ...
    const insertQuery='INSERT INTO shops (mechanic_id,shop_name,region,district,contact,worktype,expert,about) VALUES(?,?,?,?,?,?,?,?)';
...
app.get('/individual/:id',(req,res)=>{
    ...
    db.query('SELECT *FROM mechanics WHERE id=?',[id],(error,result)=>{
        if(error){
            console.error("Error executing query" ,error);
            res.status(500).json({error: "Failed to fetch person details"})
        }
...
app.post('/api/login',(req,res)=>{
    ...
    const query=`SELECT *FROM customers WHERE name='${name}' AND password='${password}'`;
    db.query(query,(err,result)=>{
        if (err){
